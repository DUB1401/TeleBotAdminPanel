...
def OpenExchanger(bot: TeleBot, user: UserData, cacher: TeleCache):
	"""
	...
	bot.send_animation(
		chat_id = user.id,
		animation = cacher.get_real_cached_file(
...
class Options:
	...
	@property
	def additional_menu(self) -> list[int]:
		"""Последовательность ID удаляемых дополнительных опций."""
	...
	def add_additional_menu(self, message: int):
		"""
		Добавляет ID сообщений или конкретного сообщения в набор сообщений дополнительных опций.

		:param messages: ID одного или нескольких сообщений.
		:type messages: int | Iterable[int]
		"""

		self.__Data["additional_menu"] = message
		self.save()
	...
	def delete_additional_menu(self, bot: TeleBot):
		"""
		Удаляет сообщения из содержащегося в параметрах списка.
		...
		TeleMaster(bot).safely_delete_messages(self.__User.id, self.__Data["additional_menu"])
		self.__Data["additional_menu"] = None
		self.save()
	...
	def inline_keyboards(self):
		...
		def Start(Call: types.CallbackQuery):
			...
			if not Subscription().IsSubscripted(User): 
				...
				return
			TeleMaster(bot).safely_delete_messages(Call.from_user.id, Call.message.id)
			OpenExchanger(bot, User, self.__Exchanger.cacher)
		...
		def Close(Call: types.CallbackQuery):
			...
			TeleMaster(bot).safely_delete_messages(Call.from_user.id, Call.message.id) 
			UserOptions.delete_removable_messages(bot)
			UserOptions.delete_additional_menu(bot)
			bot.send_animation(
...
class Exchanger:
	...
	def __init__(self, bot: TeleBot, users: UsersManager, cacher: TeleCache):
		"""
		Модуль обмена энергией.
	...
	def open(self, user: UserData):
		...
		OpenExchanger(self.__Bot, user, self.__cacher)
